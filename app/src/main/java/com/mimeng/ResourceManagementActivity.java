...
public class ResourceManagementActivity extends BaseActivity {
        ...
        if (requestCode == 1 && resultCode == Activity.RESULT_OK) {
            ...
            try (InputStream inputStream = getContentResolver().openInputStream(uri);
                ...
                while ((entry = zipInputStream.getNextEntry()) != null) {
                    ...
                    if (file.getName().equalsIgnoreCase("info.json")) {
    ...
    private void getAllResultFile(Uri uri) {
        try {
            ...
            File privateDir = new File(Objects.requireNonNull(getApplicationContext().getExternalFilesDir(null)).getAbsoluteFile() + "/");
            ...
            while (zipEntry != null) {
                ...
                if (entryName.startsWith(Objects.requireNonNull(file2.getParent())) && !zipEntry.isDirectory()) {

                    String nextPath = "resources" + entryName.replaceAll(".*(?=/[^/]*$)","");
                    File newFile = new File(privateDir, nextPath);

                    if (entryName.substring("src/".length()).startsWith("urces/res/")){
                        Log.e("getAllResultFile: ", "路径："+entryName.replaceAll(".*(?=/[^/]*$)",""));
